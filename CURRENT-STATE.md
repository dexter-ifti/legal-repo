...
- Object storage abstraction (`IStorageProvider`, `R2StorageProvider` on Cloudflare R2) — cloud-only; local disk storage is not supported and all storage operations fail closed separate from database
- Server-side multi-tenant authorization middleware (`buildTenantWhereClause`, `authorizeResourceOwnership`)
- Signup-per-tenant model: self-signup provisions a dedicated tenant (creator = ADMIN); joining an existing organization requires a single-use invite link generated by an Admin (`FRONTEND_URL` env drives the link base)
...
- Tenant provisioning policy: **every self-signup creates a dedicated new organization with the creator as ADMIN**. Joining an existing organization is possible via the invite flow: Admins generate single-use, 7-day-expiry signup links (Settings → Organization) built from the `FRONTEND_URL` env var; recipients get a pre-filled signup page (`/signup?invite=<token>`) and join with the invited role. Demo users are provisioned into the deterministic demo organization.
- User membership with roles (`ADMIN`, `MEMBER`, `ADVOCATE`, `CLERK`) and APIs: `GET /api/v1/organizations/me/members`, ADMIN-only `PATCH /api/v1/organizations/me/members/:userId` for role changes (self-demotion guard prevents removing an org's last admin), and ADMIN-only `POST /api/v1/organizations/me/invites` plus public `GET /api/v1/invites/validate/:token`.
...
- Private Cloud Object Storage Abstraction (`storage.service`, `R2StorageProvider`; cloud-only, fail-closed).
